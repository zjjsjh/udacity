...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for sentence in target_text.split('\n'):
        sentence_id = [target_vocab_to_int[word] for word in sentence.split()]
...
def model_inputs():
    ...
    targets = tf.placeholder(tf.int32, shape=(None, None), name='targer')
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    dec_input = tf.concat([go, ending], 1)
    return dec_input
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    mrnn = tf.contrib.rnn.MultiRNNCell([drop] * num_layers)
    _, encoding_state = tf.nn.dynamic_rnn(mrnn, rnn_inputs, dtype=tf.float32)
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    train_pred, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(drop, decoder_fn, dec_embed_input, sequence_length, scope=decoding_scope)     
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    logits, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell, decoder_fn, scope=decoding_scope)
    return logits
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    with tf.variable_scope('decoding') as decoding_scope:
        ...
        train_logits = decoding_layer_train(encoder_state, mrnn, dec_embed_input,
                                            sequence_length, decoding_scope, output_fn,
                                            keep_prob)    
        decoding_scope.reuse_variables()        
        infer_logits = decoding_layer_infer(encoder_state, mrnn, dec_embeddings,
